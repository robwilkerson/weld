          ...
          CompareFiles: async () => {
            ...
            return {
              lines: [
                ...
                { type: 'same', leftNumber: 1, rightNumber: 1, content: 'package main' },
                { type: 'same', leftNumber: 2, rightNumber: 2, content: '' },
                { type: 'added', leftNumber: null, rightNumber: 3, content: '// New comment added' },
                { type: 'same', leftNumber: 3, rightNumber: 4, content: '' },
                
                // Second chunk: removed/added function
                { type: 'same', leftNumber: 4, rightNumber: 5, content: 'import "fmt"' },
                { type: 'same', leftNumber: 5, rightNumber: 6, content: '' },
                { type: 'removed', leftNumber: 6, rightNumber: null, content: 'func oldFunction() {}' },
                { type: 'added', leftNumber: null, rightNumber: 7, content: 'func newFunction() {}' },
                { type: 'same', leftNumber: 7, rightNumber: 8, content: '' },
                
                // Third chunk: modified lines
                { type: 'modified', leftNumber: 8, rightNumber: 9, content: 'func main() {' },
                { type: 'modified', leftNumber: 9, rightNumber: 10, content: '  fmt.Println("Updated")' },
                { type: 'modified', leftNumber: 10, rightNumber: 11, content: '}' },
              ]
            };
          },
          GetMinimapVisible: async () => true,
          GetInitialFiles: async () => ['', ''],
          HasUnsavedChanges: async () => ({ hasUnsavedLeft: false, hasUnsavedRight: false }),
...
test.describe('Keyboard Navigation', () => {
  ...
  test('navigates between diffs with j/k keys', async ({ page }) => {
    ...
    const firstLineNumber = await page.locator('.right-pane .current-diff .line-number').first().textContent();
    ...
    const secondLineNumberLeft = await page.locator('.left-pane .current-diff .line-number').first().textContent();
    const secondLineNumberRight = await page.locator('.right-pane .current-diff .line-number').first().textContent();
    ...
    const thirdLineNumber = await page.locator('.left-pane .current-diff .line-number').first().textContent();
    ...
    const backSecondLineNumberLeft = await page.locator('.left-pane .current-diff .line-number').first().textContent();
    ...
    const backFirstLineNumber = await page.locator('.right-pane .current-diff .line-number').first().textContent();
  ...
  test('navigates with arrow keys', async ({ page }) => {
    ...
    const secondLineNumber = await page.locator('.left-pane .current-diff .line-number').first().textContent();
  ...
  test('handles navigation boundaries', async ({ page }) => {
    ...
    const stillFirstLineNumber = await page.locator('.right-pane .current-diff .line-number').first().textContent();
    ...
    const lastLineNumber = await page.locator('.left-pane .current-diff .line-number').first().textContent();
    ...
    const stillLastLineNumber = await page.locator('.left-pane .current-diff .line-number').first().textContent();
  ...
  test('shows visual feedback for current diff', async ({ page }) => {
    ...
    const line3RightPane = await page.locator('.right-pane .line .line-number').filter({ hasText: '3' }).locator('..').first();
    const line3Classes = await line3RightPane.evaluate(el => Array.from(el.classList));
  ...
  test('active diff is visually distinct from inactive diffs', async ({ page }) => {
    // Get all diff chunks (not 'same' lines)
    const allDiffChunks = await page.locator('.line-added, .line-removed, .line-modified').all();
    ...
    for (const chunk of allDiffChunks) {
      const hasCurrentDiff = await chunk.evaluate(el => el.classList.contains('current-diff'));
    ...
    const firstDiffClasses = await firstDiffLine.evaluate(el => Array.from(el.classList));
    ...
    const currentDiffBg = await currentDiffElement.evaluate(el => 
      window.getComputedStyle(el).backgroundColor
    );
    
    const inactiveDiffElement = await page.locator('.line-added:not(.current-diff), .line-removed:not(.current-diff), .line-modified:not(.current-diff)').first();
    const inactiveDiffBg = await inactiveDiffElement.evaluate(el => 
      window.getComputedStyle(el).backgroundColor
    );
